...
Аварийното възстановяване, което създава това доказателство, е възстановяването
на място на точно този архив, който `inspect` назовава:
...
Издание без миграция никога не стига до това състояние. То се връща назад само,
защото базата, която възстановяването би върнало, изобщо не е била променяна.

### Когато самото обновяване е повредено

Два пъти дефект е бил в механизма за обновяване, а не в това, което той
инсталира, и двата пъти поправката не е можела да пристигне по пътя, който
поправя: повреденото копие е това, което вече е инсталирано и върши работата.
1.4.0 и 1.4.1 отказваха всяко издание с комплект за Windows, а 1.4.2 и 1.4.3
разархивират дървото на изданието нечетимо, когато активирането се изпълнява
от agent.

Пътят офлайн е изходът и не е заобикаляне. Човек събира файловете и изпълнява
активирането от обвивка, която не използва нищо от повреденото. Така 1.4.0
стигна до 1.4.2 и така 1.4.3 щеше да мине без произшествие.

Стигнете до работещо издание офлайн и оттам обновявайте отново онлайн.

### Защо едно activation е спряло

Всяко activation записва какво е направило в дневник, достъпен само за root,
по един на опит:
...
Последните редове назовават какво е отказало. `recover-release-activation.sh
inspect` показва края на най-новия, а неуспешното activation го записва и в
дневника на agent:
...
Файлът е само за root, защото съдържа пътища, контролни суми и идентичности на
образи.

### Ако обектът работи виртуализиран

Моментна снимка на хипервайзора, направена преди обновяване, е най-бързото
възстановяване, а при прекъснато activation често е по-добра от поддържаните
команди: връща целия уред в последователно предишно състояние за около минута,
без аварийно възстановяване на базата и без заключване за изчистване.

Не замества архивите. Снимката връща всичко, включително клиничната работа,
записана след нея, затова е правилният отговор само докато обновяването е
единственото случило се оттогава. Правете я непосредствено преди обновяване и
я изтривайте, щом изданието се докаже.
