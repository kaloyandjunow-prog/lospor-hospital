...
- Прегледът в Status е зелен или всяка жълта и червена карта има отговорник. От
  конзолата: `sudo losporctl status`.
...
- Обновяванията за сигурност на хоста са инсталирани
  (`sudo unattended-upgrade --dry-run` не показва чакащи), а хостът е
  рестартиран, ако Ubuntu го е поискал.
