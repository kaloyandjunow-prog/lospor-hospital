...
for name in installed-release-state.sh operator-locale.sh update-pipeline-lib.sh terminology-agent-lib.sh site-config.sh maintenance-agent-lib.sh update-agent-loop.sh cancel-update-request.sh; do cp "$root/scripts/$name" "$scripts/$name"; done
