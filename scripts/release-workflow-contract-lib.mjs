...
export function assertReleaseWorkflowContract(candidate, publisher, quality) {
  ...
  const releaseTokenUses = publisherSecrets.filter(name => name === "HOSPITAL_RELEASE_TOKEN")
  if (releaseTokenUses.length !== 1) throw new Error("Publisher must use the scoped Hospital release token exactly once")
