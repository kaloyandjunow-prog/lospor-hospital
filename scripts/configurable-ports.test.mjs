...
test("the defaults are the documented ports", async () => {
...
test("port 80 stays put whatever the environment says", async () => {
...
test("the Status page stays bound to loopback at any port", async () => {
  ...
  // setting a port.
  const ports = await publishedPorts({ HOSPITAL_STATUS_PORT: "9443" })
...
test("the clinical HTTPS port is reachable from off-host", async () => {
  ...
  const ports = await publishedPorts({ HOSPITAL_HTTPS_PORT: "8443" })
