...
# Stamped with the release the database is on right now, not with this
# candidate. During an activation this script runs from inside the candidate
# tree, so everything in scope already names the incoming version, while the
# database being dumped is still exactly what the installed release left. A
# manifest naming the candidate is rejected by restore.sh as newer than the
# appliance, and by recover-release-activation.sh as not the journal's prior
# release -- the two consumers that exist for this one backup.
