...
export function preAuthLocale(deviceValue: unknown, applianceValue: unknown): Locale { return normalizeLocale(deviceValue, normalizeLocale(applianceValue)) }
export function localeFromSessionUser(user: unknown, fallback: Locale = DEFAULT_LOCALE): Locale {
  if (!user || typeof user !== "object") return fallback
  const record = user as { preferredLocale?: unknown; preferences?: { ui?: { locale?: unknown } } | null }
  return normalizeLocale(record.preferences?.ui?.locale ?? record.preferredLocale, fallback)
}
