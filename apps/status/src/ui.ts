...
export const CODE_MESSAGE: Record<string, string> = {
  ...
  UPDATE_SUPPLY_OFFLINE: "Offline update supply is selected. GitHub and GHCR credentials are not required.",
  UPDATE_SUPPLY_MODE_INVALID: "The update supply mode is invalid; no update route can be considered ready.",
  UPDATE_CREDENTIALS_READY: "Connected update supply has safe root-owned GitHub Releases and GHCR read credentials.",
  UPDATE_CREDENTIALS_MISSING: "Connected update supply is missing both the GitHub Releases and GHCR read credentials.",
  UPDATE_GITHUB_CREDENTIAL_MISSING: "Connected update supply is missing a safe GitHub Releases read credential.",
  UPDATE_GHCR_CREDENTIAL_MISSING: "Connected update supply is missing a safe GHCR username or read token.",
...
export const CODE_MESSAGE_BG: Record<string, string> = {
  ...
  UPDATE_SUPPLY_OFFLINE: "Избрано е офлайн предоставяне на обновявания. Не са необходими данни за достъп до GitHub и GHCR.",
  UPDATE_SUPPLY_MODE_INVALID: "Режимът за предоставяне на обновявания е невалиден; няма готов маршрут за обновяване.",
  UPDATE_CREDENTIALS_READY: "Свързаното обновяване има безопасни root данни за четене от GitHub Releases и GHCR.",
  UPDATE_CREDENTIALS_MISSING: "За свързаното обновяване липсват данните за четене както от GitHub Releases, така и от GHCR.",
  UPDATE_GITHUB_CREDENTIAL_MISSING: "За свързаното обновяване липсва безопасен токен за четене от GitHub Releases.",
  UPDATE_GHCR_CREDENTIAL_MISSING: "За свързаното обновяване липсва безопасно потребителско име или токен за четене от GHCR.",
...
const COMPONENT_LABEL_BG: Record<string, string> = {
  ...
  "update-credentials": "Данни за достъп до обновявания",
...
export function renderControlPlane(
  ...
  const accountOptions = research?.accounts.map(account =>
    `<option value="${escapeHtml(account.id)}">${escapeHtml(account.name)} — ${escapeHtml(account.email)} — ${escapeHtml(researchAccountRoleLabel(account, locale))}</option>`
  ).join("") ?? ""
  ...
  const headerActions = `<div class="header-actions">${languageSwitcher(locale, "/status/control")}<form class="logout" method="post" action="/status/logout"><button type="submit">${localize(locale, "Sign out", "Изход")}</button></form></div>`
  return page(
    localize(locale, "Hospital controls", "Управление на болничната система"),
    `<div class="shell"><header class="top"><div><div class="brand">LOSPOR Hospital</div><div class="subbrand">${localize(locale, "Research, Central, clinical guidance and external AI", "Изследвания, Central, клинични насоки и външен ИИ")}</div></div>${headerActions}</header><main>${error ? `<div class="error" role="alert">${escapeHtml(error)}</div>` : ""}${notice ? `<div class="notice" role="status">${escapeHtml(notice)}</div>` : ""}<div class="banner warn" role="status"><span class="dot" aria-hidden="true">!</span><strong>${localize(locale, "This Status login grants no clinical or research data access. It only performs the explicit control shown in each form.", "Този вход в страницата за състояние не дава достъп до клинични или изследователски данни. Той изпълнява само изричното действие във всеки формуляр.")}</strong></div>
    ...
    <p><a href="/status/">${localize(locale, "Back to status", "Назад към състоянието")}</a></p></main><footer class="foot">${localize(locale, "No enrollment token, password, AI credential, sealed credential value, certificate contents or clinical record is stored or displayed by this page.", "Тази страница не съхранява и не показва токен за свързване, парола, данни за достъп до ИИ, защитената им стойност, съдържание на сертификат или клиничен запис.")}</footer></div>`,
    locale,
  )
}

export function renderDashboard(data: DashboardData, locale: StatusLocale = "bg"): string {
...
export function renderRelease(view: ReleaseView, locale: StatusLocale = "bg"): string {
...
export function renderTerminology(view: TerminologyView, locale: StatusLocale = "bg"): string {
  ...
  const headerActions = `<div class="header-actions">${languageSwitcher(locale, "/status/terminology")}<form class="logout" method="post" action="/status/logout"><button type="submit">${localize(locale, "Sign out", "Изход")}</button></form></div>`
  return page(
    localize(locale, "Hospital terminology management", "Управление на терминологията"),
    `<div class="shell"><header class="top"><div><div class="brand">LOSPOR Hospital</div><div class="subbrand">${localize(locale, "Governed terminology generations", "Управлявани поколения терминология")}</div></div>${headerActions}</header><main>${notice}${error}<section class="section" aria-labelledby="term-active"><h2 id="term-active">${localize(locale, "Active approved generation", "Активно одобрено поколение")}</h2><div class="card">${active}${pending}${agent}</div></section><section class="section" aria-labelledby="term-actions"><h2 id="term-actions">${localize(locale, "Supported workflow", "Поддържан процес")}</h2><div class="card">${actions}</div></section><p><a href="/status/">${localize(locale, "Back to status", "Назад към състоянието")}</a></p></main><footer class="foot">${localize(locale, "This page handles bounded operational intent and approved-package provenance only. It has no shell, database, patient-data, source-file or credential access.", "Тази страница обработва само ограничени оперативни заявки и произхода на одобрения пакет. Тя няма достъп до команден ред, база данни, данни за пациенти, изходни файлове или данни за вход.")}</footer></div>`,
    locale,
  )
}
