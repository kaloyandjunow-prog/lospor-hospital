...
export function renderControlPlane(
  ...
  const ehrTransportControls = ehrTransport ? `
    ...
    <form method="post" action="/status/control/ehr-transport/policy"><label for="ehr-transport-select">${localize(locale, "EHR import transport", "Транспорт за внос на ЕЗД")}</label><select id="ehr-transport-select" name="transport"><option value="" ${!ehrTransport.transport ? "selected" : ""}>${localize(locale, "None (adapter disabled)", "Няма (адаптерът е изключен)")}</option>${ehrTransportOption("FOLDER", "Watched folder", "Наблюдавана папка")}${ehrTransportOption("FHIR", "FHIR", "FHIR")}${ehrTransportOption("HL7V2", "HL7v2", "HL7v2")}</select><label>${localize(locale, "Transport change reason", "Причина за промяната на транспорта")}<input name="reason" minlength="10" maxlength="1000" required></label><label>${localize(locale, "Administrator password", "Администраторска парола")}<input name="password" type="password" autocomplete="current-password" maxlength="256" required></label><button type="submit">${localize(locale, "Save transport policy", "Запазване на политиката за транспорта")}</button></form></div>
