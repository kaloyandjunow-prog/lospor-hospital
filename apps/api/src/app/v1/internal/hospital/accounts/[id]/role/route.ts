...
import { changeHospitalClinicalRole, hospitalClinicalRoleChangeSchema } from "@/lib/hospital/account-authority"
import {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const input = hospitalClinicalRoleChangeSchema.parse(await boundedJson(request))
    return NextResponse.json(await changeHospitalClinicalRole(prisma, id, input), {
