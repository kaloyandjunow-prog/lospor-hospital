...
export async function POST(req: NextRequest) {
  ...
  const request = await prisma.$transaction(async tx => {
    const created = await tx.roleRequest.create({ data: { userId: user.id } })
    await logAuditInTransaction(tx, user.id, "ROLE_REQUEST_SUBMIT", user.id, {
      requestId: created.id,
      requestedRole: "HEAD_OF_DEPT",
    })
    return created
  })
