...
export function useVitalDraftEntry({
  ...
  setVitalCell: (col: number, key: keyof VitalsEntry, raw: string) => void
  markVitalColDirty: (col: number) => void
  cvpUnit: "mmHg" | "cmH2O"
}) {
  ...
  const hasBlockingDraft = useCallback((col: number, key: keyof VitalsEntry) => {
