...
describe("appliance guidance policy", () => {
  it("opens a drug as manual entry with no number but keeps route and provenance", () => {
    const flyout = drugFlyout({
      guidanceEnabled: false,
      quickDoses: { Propofol: [50, 100, 150] },
      surfaces: realSurfaces({
        guidanceEnabled: false,
        adultDoseProfiles: [adultRule({
          itemKey: "Propofol",
          ruleKey: "drug.propofol",
          ruleVersion: "7",
          profile: doseProfile({
            unit: "mg",
            routes: ["IV", "IM"],
    ...
    expect(flyout?.quickDoses).toEqual([])
    expect(flyout?.calculationBasis).toBeUndefined()
    expect(flyout?.calculationWeightKg).toBeUndefined()
  })

  it("opens an infusion with zero manual rate and no suggested quick rates", () => {
    const flyout = drugFlyout({
      guidanceEnabled: false,
      mode: "infusion",
      name: "Noradrenaline",
      infusionConfigs: {
        Noradrenaline: {
          units: ["mcg/kg/min"], min: 1, max: 20, step: 0.5,
          color: "#123456", suggestedRate: 5,
        },
      },
      quickRates: { Noradrenaline: [1, 2, 5] },
    })
    expect(flyout?.rate).toBe(0)
    expect(flyout?.manualEntryOnly).toBe(true)
    expect(flyout?.quickRates).toEqual([])
  })
})
