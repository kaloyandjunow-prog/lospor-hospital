...
import {
  ...
  E2E_HOD_A_USERNAME, E2E_MEMBER_A_USERNAME, E2E_MEMBER_A2_USERNAME,
  E2E_HOD_B_USERNAME, E2E_MEMBER_B_USERNAME, E2E_RESEARCH_USERNAME,
} from "./credentials"
...
async function signIn(page: Page, username: string, file: string) {
...
setup("authenticate", async ({ page }) => {
  await signIn(page, E2E_USERNAME, authFile)
  await page.context().storageState({ path: storageStateFor("admin") })
})

setup("authenticate hod-a", async ({ page }) => {
  await signIn(page, E2E_HOD_A_USERNAME, storageStateFor("hod-a"))
})

setup("authenticate member-a", async ({ page }) => {
  await signIn(page, E2E_MEMBER_A_USERNAME, storageStateFor("member-a"))
})

setup("authenticate member-a2", async ({ page }) => {
  await signIn(page, E2E_MEMBER_A2_USERNAME, storageStateFor("member-a2"))
})

setup("authenticate hod-b", async ({ page }) => {
  await signIn(page, E2E_HOD_B_USERNAME, storageStateFor("hod-b"))
})

setup("authenticate member-b", async ({ page }) => {
  await signIn(page, E2E_MEMBER_B_USERNAME, storageStateFor("member-b"))
})

setup("authenticate research", async ({ page }) => {
  await signIn(page, E2E_RESEARCH_USERNAME, storageStateFor("research"))
})
