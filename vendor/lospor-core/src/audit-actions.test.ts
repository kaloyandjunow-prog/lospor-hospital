...
describe("API-owned audit action catalog", () => {
  ...
  /**
   * The clients hand this whatever their locale store holds, and that is not one
   * shape: a bare tag, a region tag, or the cased form a platform reports. All
   * three name the same language, so all three have to reach the same label.
   */
  it("reads the language out of any of the locale forms the clients pass", () => {
    const parsed = parseAuditActionDefinitions(catalog)
    for (const locale of ["bg", "bg-BG", "BG", "bg_BG"]) {
      expect(auditActionLabel(parsed, "ACCOUNT_ACTIVATE", locale)).toBe("Активиран профил")
    }
    expect(auditActionLabel(parsed, "ACCOUNT_ACTIVATE", "en-GB")).toBe("Account activated")
  })
})
